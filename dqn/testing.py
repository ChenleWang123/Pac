...
#
# import matplotlib
# matplotlib.use("Agg")  # ✅ 使用非 GUI 后端，避免 FigureCanvas 报错
# import matplotlib.pyplot as plt
#
# from datetime import datetime
# import numpy as np
...
#                    save_results=True, results_dir="./outputs/test_results", plot=True):  # ✅ 增加 plot 参数
#     timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#
#     if save_results:
#         os.makedirs(results_dir, exist_ok=True)
...
#                     for col in range(constants.COLS):
#                         if env.pellet_grid[row][col]:
#                             cx = col * constants.TILE_SIZE + constants.TILE_SIZE // 2
#                             cy = row * constants.TILE_SIZE + constants.TILE_SIZE // 2
